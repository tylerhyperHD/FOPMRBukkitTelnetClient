...
public class BTC_ConnectionManager
{
    ...
    public void triggerConnect(final String hostname, final int port)
    {
        ...
        if (this.hostname.equals("fop.us.to")) {
            btc.writeToConsole(new BTC_ConsoleMessage("Connecting to the FreedomOp Telnet Service. Please wait...", Color.RED));
        }
        else {
    ...
    public final void updateTitle(final boolean isConnected)
    {
        ...
        if (isConnected)
        {
            if (loginName == null)
            {
                title = String.format("FreedomOpTelnetClient - %s - %s:%d", BukkitTelnetClient.VERSION_STRING, hostname, port);
            }
            else
            {
                title = String.format("FreedomOpTelnetClient - %s - %s@%s:%d", BukkitTelnetClient.VERSION_STRING, loginName, hostname, port);
            }
        }
        else
        {
            title = String.format("FreedomOpTelnetClient - %s - Disconnected", BukkitTelnetClient.VERSION_STRING);
        }
